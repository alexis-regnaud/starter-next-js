...
export type PageProps = Partial<{
  ...
  /**
   * An event that is triggered any time the carousel start to change.
   */
  onAnimationStart: () => void;
  /**
   * An event that is triggered any time the carousel finish to change.
   */
...
export const Page = React.forwardRef<HTMLDivElement, PageProps>((props, forwardedRef) => {
  ...
  React.useEffect(() => {
    if (currentPage <= pageCount - 1 && currentPage >= 0 && currentPage !== current) {
      setCurrent((current) => {
        rPrevious.current = current;
        return currentPage;
      });

      onChangePage && onChangePage(currentPage, rPrevious.current);
    }
